## Start:
Create a `migrations` folder in `./infrastructure/persistence/ent/`
```bash
mkdir ./infrastructure/persistence/ent/migrations
```
...
```bash
go run -mod=mod entgo.io/ent/cmd/ent generate --feature sql/versioned-migration ./infrastructure/persistence/ent/schema
```
...
#### Important:
all files in `./infrastructure/persistence/ent` are generated each time you run the generate command. You should just edit your schema files that are in `./infrastructure/persistence/ent/schema`. So, If you changed anything in schema you do not need to modify or remove other files, just run `generate`
...
atlas migrate diff create_user \
  --dir "file://infrastructure/persistence/ent/migrations" \
  --to "ent://infrastructure/persistence/ent/schema" \
...
running above command will create a file in `infrastructure/persistence/ent/migrations/20260204215221_create_user.sql`
...
```bash
atlas migrate hash --dir "file://infrastructure/persistence/ent/migrations"
```

run the following command before you start fixing your schema.
```bash
atlas migrate validate --dir "file://infrastructure/persistence/ent/migrations"
```
...
### Example:
In above schema in `infrastructure/persistence/ent/schema/user.go` the following field is not needed:
```go
		field.Time("created_on").
			Optional().
			Nillable(),
...
- remove it from `infrastructure/persistence/ent/schema/user.go`
...
- verify the sql in the newly generated file `infrastructure/persistence/ent/migrations/20260204225037_remove_create_on_field_from_users.sql`
```sql
